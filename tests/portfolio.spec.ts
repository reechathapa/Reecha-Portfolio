...
test('career experience cards scroll horizontally and open in the shared dialog', async ({ page }) => {
  await page.goto('/')
  const rail = page.locator('.experience-rail')
  await page.locator('#experience').scrollIntoViewIfNeeded()
  await expect(page.locator('.experience-card').first()).toBeVisible()
  const before = await rail.evaluate(element => element.getBoundingClientRect().left)
  await page.mouse.wheel(0, 2200)
  await page.waitForTimeout(700)
  const after = await rail.evaluate(element => element.getBoundingClientRect().left)
  expect(after).toBeLessThan(before)
  await page.getByRole('button', { name: /Open details for .*KPO/ }).click()
  const dialog = page.getByRole('dialog')
  await expect(dialog.getByRole('heading', { level: 2 })).toHaveText('KPO & Company')
  await expect(dialog.getByText('Dec 2020', { exact: false }).first()).toBeVisible()
